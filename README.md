...
Upload the whole `select` directory to the server and keep this layout:
...
The wrapper now auto-detects `Sequoia-X-master` beside itself, so no Windows path changes are needed.

Example deployment:

```bash
cd /opt/select/Sequoia-X-master
cp .env.example .env
uv sync
python main.py
python /opt/select/wrapper.py
```

Optional override:

```bash
export SPIDER_DIR=/opt/select/Sequoia-X-master
python /opt/select/wrapper.py
```

`.env` and the default SQLite database path are now resolved from the project root, which is safer for `cron` and `systemd`.
